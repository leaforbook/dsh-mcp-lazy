...
`@yilinxiao/dsh-mcp-lazy` 0.3.0 已能按需连接 MCP、动态注册工具、处理目录变更并有限重连，但仍有三个使用成本：模型必须记住先调用每个服务器的 `activate`；轮末释放后下一轮会重新启动服务器；插件实现直接依赖 DSH 的 `ctx.tools`、事件和 effect 生命周期。
...
- 当前实例的宿主 adapter（仅用于路由工具发布所有权，不暴露原始 service）
- `serverName`
...
2. query 包含完整 `mcp__<server>__` 前缀时先保留大小写精确选择；没有精确项时，只有大小写折叠后的候选唯一才选择。
...
共享路由工具的宿主注册由某个存活条目 adapter 持有。该条目所在 Cordis fiber 卸载时，注册所有权转交给一个存活条目；即使原生同名工具正在临时占用名称，所有权状态也随条目清理并在占用结束后通过存活 adapter 恢复。宿主 disposer 抛错时仍先完成状态转移，所有 disposer 只消费一次。
...
连接需求再分成当前轮使用者和跨轮次持有者。`releaseOnTurnEnd: true` 只保留当前轮使用者；设为 `false` 时，实际激活或调用过服务器的 agent 同时成为跨轮次持有者，轮次停止不会删除该持有权。
...
4. 保温期收到 `tools/list/changed`：刷新并替换 catalog；没有活跃使用者时不把工具重新发布到 DSH。所有目录发现使用单调代次，较早发现晚返回时不得覆盖较新的已完成刷新。
5. 保温超时：关闭 client，但保留 catalog 的名称和描述供路由评分；下次激活重新连接并重新发现目录。
6. 显式 deactivate、插件销毁：立即卸载工具并关闭连接，不等待保温计时器。
7. 意外断线：立即卸载工具。只有存在当前轮使用者、跨轮次持有者或 `autoActivate` 时才按预算重连；单纯处于保温状态不后台重连。

`releaseOnTurnEnd: false` 继续保持工具发布和连接，不进入保温状态；无关 agent 的销毁不会影响它，只有实际最后一个持有者的 `agent/disposed` 才触发释放。`autoActivate: true` 是独立的常驻所有权，继续在启动时连接并发布工具。
