...
function createServerRuntime({
  ...
  async function closeClient(target) {
    if (target === undefined) return
    try { await target.close() } catch (error) { adapter.log('warn', `${label}: close failed: ${String(error)}`) }
  }
  ...
  async function refreshCatalog(targetClient) {
    ...
    if (client !== targetClient || disposed) return
    catalog = definitions
    if (shouldPublishCatalog()) publishCatalog()
  ...
  function onAgentDisposed(payload) {
    const before = users.size
    users.delete(payload?.agent)
    if (users.size !== before) releaseWhenUnused('会话已销毁')
  }
