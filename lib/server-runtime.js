...
function createServerRuntime({
  ...
  let warmTimer
  let turnUsers = new Set()
  let persistentOwners = new Set()
  ...
  function addUser(agent) {
    if (!agent) return
    turnUsers.add(agent)
    if (!config.releaseOnTurnEnd) persistentOwners.add(agent)
  }
  ...
  function wantsConnection() {
    return turnUsers.size > 0 || persistentOwners.size > 0 || config.autoActivate
  }

  function shouldPublishCatalog() {
    return warmTimer === undefined && wantsConnection()
  }
  ...
  function beginCatalogDiscovery(targetClient, signal) {
    const state = {
      generation: ++discoveryGeneration,
      targetClient,
      promise: Promise.resolve().then(() => discoverDefinitions(targetClient, signal))
    }
    latestDiscovery = state
    return state
  }

  async function awaitNewestDiscovery(initialState) {
    let state = initialState
    while (true) {
      const definitions = await state.promise
      const newest = latestDiscovery
      if (newest === state) return { state, definitions }
      if (newest?.targetClient !== initialState.targetClient) return { state, definitions }
      state = newest
    }
  }
  ...
  async function refreshCatalog(targetClient) {
    ...
    const discovery = beginCatalogDiscovery(targetClient)
    const definitions = await discovery.promise
    if (latestDiscovery !== discovery || client !== targetClient || disposed) return
  ...
  async function activate(agent, automatic = false, externalSignal) {
    ...
    activation = (async () => {
      ...
      try {
        ...
        client = gen
        connectingClient = undefined
        const initialDiscovery = beginCatalogDiscovery(gen, activationAbort.controller.signal)
        const { state: completedDiscovery, definitions } = await awaitNewestDiscovery(initialDiscovery)
        if (client !== gen) throw new Error(`${label}: connection closed during activation`)
        if (disposed) throw new Error(`${label}: plugin disposed during activation`)
        if (activationAbort.controller.signal.aborted) throw activationAbort.controller.signal.reason
        if (!wantsConnection()) throw new Error(`${label}: activation no longer needed`)
        if (latestDiscovery !== completedDiscovery) throw new Error(`${label}: catalog discovery was superseded`)
  ...
  function onTurnStopping(payload) {
    const removed = turnUsers.delete(payload?.agent)
    if (!config.releaseOnTurnEnd) return
    if (removed) releaseWhenUnused('本轮对话结束，无会话继续使用')
  }

  function onAgentDisposed(payload) {
    turnUsers.delete(payload?.agent)
    persistentOwners.delete(payload?.agent)
