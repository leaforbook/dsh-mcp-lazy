...
function releaseNativeOwner(registry, owner) {
  ...
  if (registry.nativeOwner === owner) {
    ...
    if (!hasRouterSource(registry)) registries.delete(registry.identity)
    else {
...
function registerRouterCompatibleTool(adapter, definition) {
  ...
  if (definition.name !== ROUTER_TOOL_NAME) {
    ...
    try {
      ...
      let cleanupError
      try { dispose() } catch (disposeError) { cleanupError = disposeError }
      throw combineErrors(error, cleanupError, 'managed tool provenance registration failed')
    }
    let disposed = false
...
function registerRouterVisibility(adapter, controller) {
  ...
  try {
    ...
    if (!hasRouterSource(registry) && registry.nativeOwner === undefined) registries.delete(adapter.identity)
    throw error
  }
  ...
  const disposeVisibility = () => {
    if (disposed) return
    disposed = true
    if (registry.visibility !== visibility) return
    registry.visibility = undefined
    let failure
