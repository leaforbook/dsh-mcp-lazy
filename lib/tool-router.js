...
function publishSharedRouter(registry, preferredOwner) {
  if (registry.sharedPublication !== undefined || registry.nativeOwner !== undefined || registry.entries.size === 0) return
  const owner = preferredOwner !== undefined && registry.entries.get(preferredOwner.entry.serverName) === preferredOwner
    ? preferredOwner
    : firstEntryRecord(registry)
...
function releaseNativeOwner(registry, owner) {
  ...
  if (registry.nativeOwner === owner) {
    ...
    if (registry.entries.size === 0) registries.delete(registry.identity)
    else {
...
function registerRouterServer(adapter, entry) {
  ...
  if (registry === undefined) {
    ...
    registry.definition = {
      ...
      execute: async (args, exec) => {
        const route = selectRoute([...registry.entries.values()].map((record) => record.entry), args)
  ...
  return () => {
    if (disposed) return
    disposed = true
    if (registry.entries.get(entry.serverName) !== record) return
    registry.entries.delete(entry.serverName)
    let failure
    ...
    if (registry.entries.size === 0 && registry.nativeOwner === undefined) {
