...
import { registerRouterServer } from './tool-router.js'
...
async function apply(ctx, config) {
  ...
  adapter.on('agent/turn-stopping', runtime.onTurnStopping)
  adapter.on('agent/disposed', runtime.onAgentDisposed)

  adapter.effect(() => {
    const activationPromise = config.autoActivate ? runtime.activate(undefined, true) : null
    return () => {
      unregisterRouter()
      void runtime.dispose()?.catch((error) => adapter.log('error', `${label}: plugin disposal failed: ${String(error)}`))
